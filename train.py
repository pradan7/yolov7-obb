...
def train(hyp, opt, device, tb_writer=None):
    ...
    callback_logger = Loggers(save_dir, weights, opt, hyp, logger)
    for k in methods(callback_logger):
        callbacks.register_action(k, callback=getattr(callback_logger, k))
    # Model
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size // opt.world_size, gs, names, opt.single_cls,
                                              hyp=opt.hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz, batch_size // opt.world_size * 2, gs, names, opt.single_cls,
                                       hyp=opt.hyp, cache=None if opt.notest else opt.cache_images, rect=True, rank=-1,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _) in pbar:  # targets => target image index, class, x, y , normalized w, normalized h.
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint4567 to float32, 0-255 to 0.0-1.0
