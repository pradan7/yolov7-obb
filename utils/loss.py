...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))
        self.box_loss = nn.MSELoss(reduction='mean')
        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
        self.cp, self.cn = smooth_BCE(eps=h.get('label_smoothing', 0.0))  # positive, negative BCE targets
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        lbox, lobj = self.calculate_losses(p, targets, imgs)
        if self.autobalance:
            self.balance = [x / self.balance[self.ssi] for x in self.balance]
        lbox *= self.hyp['box']
        lobj *= self.hyp['obj']
        loss = lbox + lobj
        # lcls = torch.Tensor([0]).cuda().half() # Placeholder
        bs = p[0].shape[0]
        if type(lbox)!=torch.Tensor:
            lbox = torch.Tensor(lbox)
        if type(lobj)!=torch.Tensor:
            lobj = torch.Tensor(lobj)
        if type(loss)!=torch.Tensor:
            loss = torch.Tensor(loss)
        return loss * bs, torch.cat((torch.unsqueeze(lbox,0), torch.unsqueeze(lobj,0), torch.unsqueeze(loss,0))).detach()
    ...
    # def find_3_positive(self, p, targets):
        ...
        #         # r = t[:, :, 4:6] / anchors[0, None]  # wh ratio
    ...
    def calculate_losses(self, p, targets,imgs):
        # p shape => (16, 1, 80/40/20, 80/40/20, 83) 
        # targets => (number of targets in current batch, 6) or (n_targets , [batch_image_number, class_idx, x, y, w, h])
        ...
        for i in range(self.nl):
            ...
            box_loss = 0 
            object_loss = 0
            # n_grid_rows, n_grid_columns = p[i].shape[2:4]
            # grid_diagonal_len = math.sqrt((0.5 + image_height/n_grid_rows)**2 + (0.5 + image_width/n_grid_columns)**2)
            
            for batch_idx in range(batch_size):
                b_idx = (targets[:, 0]==batch_idx)
                target_for_batch_idx = targets[b_idx]
                if target_for_batch_idx.shape[0]==0:
                    continue
                if len(target_for_batch_idx)==0:
                    continue
                # Match targets to anchors
                t = target_for_batch_idx * gain
                # print("targets = ",targets.shape)
                if n_targets:
                    # Matches
                    t = t.reshape((-1,6))
                ...
                # gi = gi.tolist()
                # gj = gj.tolist()      
                # grid_dict = {} # To handle duplicate grids. 
                # for (gridx, gridy) in zip(gi,gj): 
                #     try:
                #         grid_dict[(gridx,gridy)] +=1 
                #     except:
                #         grid_dict[(gridx,gridy)] = 0
                # gi = [gridx for gridx,_ in grid_dict.keys()]
                # gj = [gridy for _,gridy in grid_dict.keys()]
                # gi = torch.Tensor(gi).to(torch.long).to(device=targets.device)#, device = targets.device)
                # gj = torch.Tensor(gj).to(torch.long).to(device=targets.device)#, device = targets.device)
                
                # inverse_gi = [i for i in range(n_grid_rows) if i not in gi]
                # inverse_gj = [i for i in range(n_grid_columns) if i not in gj]
                # print(grids_with_no_preds)
                # grids_with_no_preds = torch.zeros((n_grid_rows,n_grid_columns))

                # grids_with_no_preds[gi,gj] = 1
                target_object_scores = torch.full_like(p[i][batch_idx,...,2],0.01)
                target_object_scores[0, gi, gj] = 0.99 #Only set matched objects to 0.99, others are still 0.01
                matched_preds = p[i][batch_idx,:,gi, gj,:]
                grid = torch.stack([gi, gj], dim=1)
                pxy = matched_preds[..., :2].sigmoid() * 2. - 0.5
                # pxy = torch.squeeze(pxy)
                gxy -= grid #TODO: why?
                # matched_predicted_object_score = matched_preds[...,2] # Predicted object score. 
                # matched_target_object_score = torch.full_like(matched_predicted_object_score, 0.99) # 0.99 as target. (Same logic as label smoothing.)
                # Calculate Objectness BCE loss to optimize objectness score. TODO: Figure out a logic so that objectness converges.
                # TODO: calculate object loss for unmatched detections.
                object_loss = (object_loss*batch_idx + self.BCEobj(p[i][batch_idx,:,:,:,2], target_object_scores) )/(batch_idx+1)
                # Calculate MSE as error for center detection.
                box_loss = (box_loss*batch_idx + self.box_loss(torch.squeeze(pxy),torch.squeeze(gxy)))/(batch_idx+1) # Adding MSE of each element in a batch.
            batch_object_loss+= object_loss*self.balance[i] # Adding object loss of each batch of images.
            if self.autobalance:
                self.balance[i] = self.balance[i] * 0.9999 + 0.0001 / (object_loss/batch_size).detach().item() #TODO: ??
            batch_box_loss += box_loss # Adding MSE of each batch of images.
        complete_object_loss = batch_object_loss/self.nl # Average object loss across all layers/scales.
        complete_box_loss = batch_box_loss/self.nl # Average MSE across all batches or scales.
        # returns (batch indexes which match, all anchors which match (always 0), matching gridx and y, matching targets, all anchor boxes)

        return complete_box_loss, complete_object_loss           
...
class ComputeLossBinOTA:
    ...
    #         pair_wise_iou_loss = -torch.log(pair_wise_iou + 1e-8)
    #         # print("pwcl = ",pair_wise_iou_loss.shape)
    #         top_k, _ = torch.topk(pair_wise_iou, min(10, pair_wise_iou.shape[1]), dim=1)
    #         dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
